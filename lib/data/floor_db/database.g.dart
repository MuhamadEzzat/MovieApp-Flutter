...
class _$AppDatabase extends AppDatabase {
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Movie` (`id` INTEGER, `overview` TEXT, `title` TEXT, `poster_path` TEXT, `is_fav` INTEGER, PRIMARY KEY (`id`))');
  ...
  MovieDao get movieDao {
    return _personDaoInstance ??= _$MovieDao(database, changeListener);
  }
}
  ...
  Future<List<Movie>> findAllMovies() async {
    return _queryAdapter.queryList('SELECT * FROM Movie',
        mapper: (Map<String, Object?> row) => Movie(
            ...
            is_fav:
                row['is_fav'] == null ? null : (row['is_fav'] as int) != 0));
  }
